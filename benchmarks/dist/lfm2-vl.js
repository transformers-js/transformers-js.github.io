...
var TILE_SIZE = 512;
var PATCH_SIZE = 16;
var PATCHES_PER_SIDE = TILE_SIZE / PATCH_SIZE;
var MAX_PATCHES = PATCHES_PER_SIDE * PATCHES_PER_SIDE;
var PATCH_DIM = PATCH_SIZE * PATCH_SIZE * 3;
...
async function tileToPatches(image) {
...
async function preprocessVLImage(image, maxTiles = 10, useThumbnail = false) {
...
var LFM2VLForConditionalGeneration = class _LFM2VLForConditionalGeneration {
  constructor(embedImages, embedTokens, decoder, tokenizer, modelCfg, eosTokenId, imageTokenId, maxTiles, useThumbnail, hasPositionIds, hiddenSize, decoderInputNames) {
  ...
  static async fromHub(modelId, options = {}) {
    ...
    const config = await fetchJSON(modelId, "config.json", mirrorBaseUrl);
  ...
  async chat(messages, image, options = {}) {
    ...
    const t0 = t?.();
    const { pixelValues, pixelAttentionMask, spatialShapes, numTiles } = await preprocessVLImage(image, this.maxTiles, this.useThumbnail);
    const t1 = t?.();
    const MAX_PATCHES2 = 1024, PATCH_DIM2 = 768;
    const imgOut = await this.embedImages.run({
      pixel_values: { data: pixelValues, dims: [numTiles, MAX_PATCHES2, PATCH_DIM2] },
      pixel_attention_mask: { data: pixelAttentionMask, dims: [numTiles, MAX_PATCHES2] },
